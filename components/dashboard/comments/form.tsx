            ...
            <form ref={ref} onSubmit={handleSubmit(onSubmitHandle)}
                ...
                <div className='flex flex-col gap-4'>
                    ...
                    <div className='flex flex-col gap-2'>
                        ...
                        <Input  {...register("occupationRu", { required: true })} autoComplete='off' placeholder='Ru' className={`text-neutral-500 ${errors.occupationRu ? 'border-destructive focus-visible:!border-destructive' : ''}`} />
                    </div>
                    
                    <div className='flex flex-col gap-2'>
                        <Input {...register("occupationUz", { required: true })} autoComplete='off' placeholder='Uz' className={`text-neutral-500 ${errors.occupationUz ? 'border-destructive focus-visible:!border-destructive' : ''}`} />
                    </div>
                    <div className="flex flex-col gap-2">
                        <label htmlFor="отзыв" className='font-normal text-sm text-neutral-400'>Oтзыв</label>
                        <Textarea {...register("textRu", { required: true, onChange: (e) => { setcount1(e.target.value.length) } })} maxLength={400} placeholder='Ru' autoComplete='off' className={`text-neutral-500 h-[120px] ${errors.textRu ? 'border-destructive focus-visible:!border-destructive' : ''}`}  />

                        <p className={`${count1 >= 400 ? 'text-red-500' : ''} text-end`}>{count1 || 0}/400</p>
                    </div>

                    <div className="flex flex-col gap-2">
                        <Textarea {...register("textUz", { required: true, onChange: (e) => { setcount2(e.target.value.length) } })} maxLength={400} placeholder='Uz' autoComplete='off' className={`text-neutral-500 h-[120px] ${errors.textUz ? 'border-destructive focus-visible:!border-destructive' : ''}`} />
