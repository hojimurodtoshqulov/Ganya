    ...
    if (banner) {
      if (method === "DELETE") {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/banners/delete/${banner.id}`,
          {
            method: "DELETE",
            headers: {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/banners/update/${banner.id}`,
          {
            method: "PATCH",
            body: formData,
            headers: {
      ...
      formData.append("link", data.link);
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/banners/create`,
        {
          method: "POST",
          body: formData,
          headers: {
      ...
      {banner ? (
        <div
          onClick={(e) => {
            if (
              e.target &&
              (e.target as Element).classList.contains("addBanner")
            ) {
              onClick();
            }
          }}
          ...
          <div className="p-10 bg-white rounded-2xl absolute z-[100]">
            <form ref={ref} onSubmit={handleSubmit(onSubmitHandle)}>
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="Ссылка"
                  className={`mt-4 pl-12 ${errors.link ? "border-destructive focus-visible:!border-destructive" : ""}`}
