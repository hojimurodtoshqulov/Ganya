...
const Form = ({open, openFunc}:{openFunc:Function, open:boolean}) => {
    ...
    return (
        <div className={`w-screen h-screen z-50 top-0 left-0 flex justify-center items-center ${open ? 'fixed':'hidden'}`} style={{ backgroundColor: "rgba(100, 121, 55, 0.2)" }} onClick={() => openFunc()} >
            <form ref={ref} onSubmit={handleSubmit(onSubmitHandle)} className='bg-white flex flex-col w-11/12 max-w-[648px] max-h-[534px] h-5/6 overflow-auto p-10 gap-6 rounded-2xl' style={{ scrollbarWidth: 'none' }}>
                <div className='flex flex-col gap-4'>
                    ...
                    <div className='flex flex-col gap-2'>
                        ...
                        <textarea  {...register("отзыв", { required: true })} ref={text1ref} onChange={() => Changed(text1ref)} maxLength={400} placeholder='Введите отзыв пользователя' className=" text-w-full rounded-xl h-[120px] text-lg outline-none p-4 resize-none border-2 placeholder-neutral-500 text-neutral-500" />
                    ...
                    <div className='flex flex-col gap-2'>
                        ...
                        <textarea {...register("Sharh", { required: true })} ref={text2ref} onChange={() => Changed(text2ref)} maxLength={400} placeholder='Foydalanuvchi sharhini kiriting' className=" text-w-full rounded-xl h-[120px] text-lg outline-none p-4 resize-none border-2 placeholder-neutral-500 text-neutral-500" />
