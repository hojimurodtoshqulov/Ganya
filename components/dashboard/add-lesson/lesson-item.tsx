...
const LessonItem: React.FC<Props> = ({ type, link, lang, value = { title: {}, number: 0 } }) => {
    // const [video, setVideo] = useState<any>(null)
    // useEffect(() => {
    //     const fetch = async () => {
    //         const req = value && (await fetch(`${value?.title.video}`,))
    //         console.log(req)
    //     }
    // }, [value?.title?.video])

    return (
        <Link href={`/${lang}/${link}/`} className={clsx("rounded-2xl p-4 flex items-center justify-start gap-3", type === 'item' && 'bg-white', type === 'create' && 'border-dashed border-2')}>
