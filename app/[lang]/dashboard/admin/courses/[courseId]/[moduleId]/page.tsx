...
const page: FC<Props> = ({ params }): JSX.Element => {
  ...
  useEffect(() => {
    const getModuleById = async () => {
      try {
        const api = process.env.BASE_URL + `/modules/single/${params.moduleId}`;
  ...
  return <div>
    ...
    <div className="flex flex-col gap-3">
      {data?.Lesson?.map((lesson: any, index: number) => (
        <LessonItem type="item" link={`/dashboard/admin/courses/${params.courseId}/${params.moduleId}/${lesson.id}`} value={{ title: lesson, number: index }} key={lesson.id} />
      ))}
      ...
      <LessonItem type="create" link={`/dashboard/admin/courses/${params.courseId}/${params.moduleId}/create/?number=${lessonsLength + 1}`} />
