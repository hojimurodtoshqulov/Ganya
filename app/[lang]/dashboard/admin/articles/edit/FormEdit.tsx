...
const FormEditArticle: FC<Props> = ({ defaultValues, articleId, }) => {
    ...
    const { control, handleSubmit, register, watch, formState: { errors, isSubmitting } } = useForm<Schema>({
    ...
    const articleImage = watch('articleImage') && (watch('articleImage')[0] ?? {});
        ...
        <div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="">
                <div className="bg-white p-6 rounded-2xl mt-5 space-y-5">
                        ...
                        <Dialog>
                            <DialogTrigger asChild>
                                <Button /* onClick={() => setModal(true)} */ variant={"filled"}>
                                    {imageMobile?.name && imageWeb?.name ? 'редактировать' : 'Выбрать'}
                                </Button>
                            </DialogTrigger>
                            <DialogContent className="p-7">
                                <div className={cn("border-dashed border-[2px] rounded-2xl p-4 flex justify-between items-center  mb-4", 'border-destructive' === errors?.imageMobile)}>
                                    <div className="flex items-center">
                                        <div className=" rounded-xl flex items-center justify-center w-14 h-14 bg-slate-500">
                                            <ImageIcon />
                                        </div>
                                        <div className="flex ml-3 flex-col gap-1">
                                            <h2 className="text-2xl font-normal"> Обложка </h2>
                                            <p className="text-base">
                                                {imageMobile ? (imageMobile?.name as string) : 'Выберите или перетащите обложку для курса'}
                                            </p>
                                        </div>
                                    </div>
                                    <label
                                    ...
                                    <Input type="file" accept="image/*" className="w-0 h-0 opacity-0 hidden" id="file1" {...register('imageMobile', { required: true })} />
                                </div>
                                <div className={cn("border-dashed border-[2px] rounded-2xl p-4 flex justify-between items-center ", 'border-destructive' === errors?.imageWeb)}>
                                    <div className="flex items-center">
                                        <div className="rounded-xl flex items-center justify-center w-14 h-14 bg-slate-500">
                                            <ImageIcon />
                                        </div>
                                        <div className="flex ml-3 flex-col gap-1">
                                            ...
                                            <p className="text-base">
                                                {imageWeb ? (imageWeb?.name as string) : 'Выберите или перетащите обложку для курса'}
                                            </p>
                                        </div>
                                    </div>
                                    <label
                                    ...
                                    <Input type="file" accept="image/*" className="w-0 h-0 opacity-0 hidden" {...register('imageWeb', { required: true })} id="file2" />
                                </div>
                                <div className="relative">
                                    <Link className="absolute top-2 left-3" />
                                    <Input placeholder="Ссылка" className="mt-4 pl-12" {...register('link', { required: true })} />
                                </div>
                                <DialogClose asChild>
                                    <Button /* onClick={() => setModal(true)} */ variant={"filled"}>
                                        Soxranit
                    ...
                    <div className={cn("border-dashed border-[2px] rounded-2xl p-4 flex justify-between items-center mt-4 mb-2", 'border-destructive' === errors?.articleImage)}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-2xl font-normal flex flex-col ">
                                ...
                                <p>{articleImage ? (articleImage?.name as string) : 'Faqat Article uchun Rasm'}</p>
                            </div>
                        </div>
                        <label className={buttonVariants({ variant: "filled" })}>
                            ...
                            <Input type="file" accept="image/*" className="w-0 h-0 opacity-0 hidden" {...register('articleImage', { required: true })} />
                        </label>
                    </div>
                    ...
                    <div className="grid w-full  items-center gap-1.5">
                        ...
                        <Textarea
                            placeholder="Преимущество 1"
                            className={cn('border-destructive' === errors?.textUz)}                {...register("textUz")}
                        />
                    </div>

                    <div className="flex sm:justify-end mt-5 justify-center">
                        <Button disabled={isSubmitting} variant={"main"} className="bg-red-500 hover:bg-red-400 transition-colors">
                            {'O\'chirish'}
                        </Button>  <Button disabled={isSubmitting} type="submit" variant={"main"}>
                            Nashr qilish
                        </Button>
                    </div>
                </div>
